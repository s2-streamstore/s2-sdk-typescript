...
console.log(`Tail seqNum=${startAt.tail.seq_num}`);


const session = await stream.appendSession({
	maxQueuedBytes: 1024 * 1024 * 1,
...
let lastAcked= session.lastAckedPosition()!.end.seq_num;
console.log("lastAcked=%s", { lastAcked })

let readSession = await stream.readSession({
	seq_num: startAt.tail.seq_num,
	count: lastAcked - startAt.tail.seq_num,
	as: "bytes",
});
