...
describeIf("ReadSession Integration Tests", () => {
	...
	it.each(transports)(
		"signals caught-up after draining the backlog (%s)",
		async (transport) => {
			const basin = s2.basin(basinName);
			const stream = basin.stream(streamName, { forceTransport: transport });
			...
			// Fires even though nothing is consuming records yet.
			const tail = await session.caughtUp();
			expect(tail.seqNum).toBeGreaterThanOrEqual(3);
			expect(session.isCaughtUp()).toBe(true);

			// Already caught up: resolves immediately.
			await expect(session.caughtUp()).resolves.toMatchObject({
				seqNum: tail.seqNum,
			});

			// Backlog records were all delivered before the caught-up signal.
			const records: number[] = [];
			const reader = session.getReader();
			while (records.length < 3) {
				const r = await reader.read();
				if (r.done) break;
				records.push(r.value.seqNum);
			}
	...
	it.each(transports)(
		"rejects caughtUp() with SESSION_CLOSED when limits end the session early (%s)",
		async (transport) => {
			const basin = s2.basin(basinName);
			const stream = basin.stream(streamName, { forceTransport: transport });
